...
def unpack_scalar_index(packed, tensor_shape, tensor_dtype, bit_width):
    unpacked_indices = unpack_index_tensor(
        packed_tensor=packed, index_bits=bit_width, num_elements=math.prod(tensor_shape)
    )
...
def unpack_index_tensor(
    packed_tensor: torch.Tensor, index_bits: int, num_elements: int
) -> torch.Tensor:
    ...
    pad_size = (packed_tensor.shape[-1] * 32) % (index_bits * num_elements)
    out = out.reshape(*packed_tensor.shape[:-1], -1)
    if pad_size > 0:
        out = out[..., :-pad_size]
    out = out.reshape(*packed_tensor.shape[:-1], -1, index_bits)
    wf1 = torch.arange(0, index_bits, 1).to(packed_tensor.device).view(1, -1)
...
def pack_index(
    ...
    assert index_bits <= 32, f"total index bits {index_bits} should be less than 32"
    ...
    # tests
    unpack_indices = unpack_index_tensor(out, index_bits, indice.shape[-1])
    ...
    assert torch.allclose(
        indice.view(index_dtype).to(torch.int64),
        unpack_index_tensor(out, index_bits, indice.shape[-1]),
    )
    return out
