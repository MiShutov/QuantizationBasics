...
class Quantizer(nn.Module):
    ...
    def configure(self, module):
        ...
        if self.use_offset is False:
            ...
            offset = (w_max * self.negative_clip - w_min * self.positive_clip) / (
                self.positive_clip - self.negative_clip
            )
...
class QuantizedLinear(QuantizedLayer):
    ...
    @torch.no_grad()
    def wrap_module(self, module):
        self.int_weight = module.weight_quantizer.get_quant(module.module.weight).to(
            torch.int8
        )
